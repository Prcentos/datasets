...
class ParaCrawlConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, text_encoder_config=None, target_language=None, **kwargs):
    ...
    assert target_language in _TARGET_LANGUAGES
...
class ParaCrawl(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    file_path = dl_manager.download_and_extract(self.builder_config.data_url)

    tf.logging.info("File path: %s", file_path)
    ...
    return [
        tfds.core.SplitGenerator(
            ...
            num_shards=10,
            gen_kwargs={"data_file": file_path})
    ]
  ...
  def _generate_examples(self, data_file):
    ...
    tf.logging.info("Data file: %s", data_file)
